...
def readInputFile(inputFileName):
	...
	for i in range(len(sheets)):
		...
		sheetValues = []
		for row in range(sheets[i].nrows):
			rowValues = []
			for col in range(sheets[i].ncols):
...
def createBarcode(previousBarcode):
	for i in range(len(previousBarcode)):
		try:
...
if __name__ == '__main__':
	inputFileName = input("Path to excel file: ")
	counterColName = input("Sort by: ")
	#############################
